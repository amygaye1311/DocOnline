...
const Navbar: React.FC = () => {
  ...
  const linkClass = ({ isActive }: { isActive: boolean }) =>
    `block px-3 py-2 rounded-md transition ${isActive ? "bg-white text-sky-600" : "text-white hover:bg-sky-500"}`;
  ...
  return (
    <nav className="bg-sky-600 text-white p-4">
      <div className="mx-auto flex max-w-7xl items-center justify-between">
        ...
        <button
          type="button"
          className="md:hidden rounded-md border border-white/25 p-2 hover:bg-sky-500"
      ...
      <div className={`${mobileOpen ? "block" : "hidden"} mt-3 md:hidden`}>
        <div className="space-y-2 rounded-xl border border-white/20 bg-sky-700/95 p-3">
